...
const loginForm =    <form method="POST" action="/admin/login">   <h2>Admin Login</h2>   <label for="username">Username:</label>   <input name="username" id="username" placeholder="Username" required />   <label for="password">Password:</label>   <input type="password" name="password" id="password" placeholder="Password" required />   <button type="submit">Login</button>   </form>;
...
return res.set('Content-Type', 'text/html').send(getHtmlTemplate('Login Failed',   <p>Login failed. <a href="/admin/login">Try again</a></p>  ));
}
...
const dashboardContent =   <h1>Welcome Admin: ${req.session.username}</h1>   <ul class="admin-links-list">   <li><a href="/admin/add-chat-replies">➕ Add Chat Reply</a></li>   <li><a href="/admin/reply-list">📃 View Replies</a></li>   <li><a href="/admin/custom-variables">🔧 Manage Custom Variables</a></li>   <li><a href="/admin/logout">🚪 Logout</a></li>   </ul>   <style>   .admin-links-list {   list-style: none;   padding: 0;   margin: 20px 0;   display: flex;   flex-direction: column;   gap: 12px;   }   .admin-links-list li a {   display: block;   background-color: #4f46e5;   color: #fff;   padding: 12px 20px;   border-radius: 6px;   font-size: 16px;   text-decoration: none;   transition: background 0.3s;   }   .admin-links-list li a:hover {   background-color: #4338ca;   }   </style>  ;
...
<select name="sendMethod" id="sendMethod">  
    ...
    <option value="once">Once (first one)</option>  
        ...
        </select>  <div id="keywordField" style="display:none;">  
...
<textarea name="replies" id="replies" required></textarea>  <button type="button" onclick="toggleVariableList()">🧠 Insert Variable</button>
...
<select name="sendMethod" id="sendMethod">  
    ...
    <option value="once" ${r.sendMethod === 'once' ? 'selected' : ''}>Once</option>  
        ...
        <h2>Edit Reply</h2>  
        <label for="type">Type:</label>  
        <input name="type" id="type" value="${r.type}" readonly />  <div id="keywordField" style="display:none;">  
        <label for="keyword">Keyword(s):</label>  
...
const listItems = variables.map(v =>   <li class="variable-item">   <div class="variable-name"><code>%${v.name}%</code></div>   <div class="variable-value">${v.value.slice(0, 80)}${v.value.length > 80 ? '...' : ''}</div>   <div class="actions">   <a href="/admin/edit-variable/${v._id}" class="edit-btn">✏️</a>   <a href="/admin/delete-variable/${v._id}" class="delete-btn" onclick="return confirm('Delete this variable?')">🗑️</a>   </div>   </li>  ).join('');
...
const editVariableForm =   <form method="POST" action="/admin/edit-variable/${variable._id}">   <h2>Edit Custom Variable</h2>   <label for="name">Variable Name (read-only):</label>   <input name="name" id="name" value="${variable.name}" readonly />   <label for="value">Variable Value:</label>   <textarea name="value" id="value" required>${variable.value}</textarea>   <button type="submit">Update Variable</button>   </form>   <br>   <a href="/admin/custom-variables">← Back to Custom Variables</a>  ;
